...
const Default: React.FC<Props> = ({ data, productId }) => {
    ...
    const [currentPrice, setCurrentPrice] = useState<number>(productMain.price);
    const [quantityPurchase, setQuantityPurchase] = useState(productMain.quantityPurchase);
    ...
    const handleColorSelect = (colorCode: string) => {
        ...
        if (selectedVariation) {
            ...
            setCurrentPrice(productMain.price);
        }
    };
    ...
    const handleIncreaseQuantity = () => {
        ...
        const selectedVariation = productMain.variation.find(
            (variation) => variation.colorCode === activeColor
        );

        const selectedPrice = selectedVariation ? selectedVariation.price : productMain.price;

        updateCart(productMain.id, productMain.quantityPurchase, activeColor, selectedPrice);
    };
    ...
    const handleAddToCart = () => {
        ...
        if (!activeColor && productMain.variation.length > 0) {
            const firstColor = productMain.variation[0].colorCode;
            setActiveColor(firstColor);
        }
    
        const colorToUse = activeColor || (productMain.variation.length > 0 ? productMain.variation[0].colorCode : '');
    
        const selectedVariation = productMain.variation.find(
            (variation) => variation.colorCode === colorToUse
        );
    
        // Ensure selectedPrice is a number
        const selectedPrice = selectedVariation
            ? parseFloat(selectedVariation.price.toString()) // Ensure price is treated as a number
            : parseFloat(productMain.price.toString()); // Default price if no variation
    
        // Ensure you're passing a number for selectedPrice
        if (!cartState.cartArray.find(item => item.id === productMain.id && item.selectedColor === colorToUse)) {
            addToCart(productMain, colorToUse, activeColor, selectedPrice); // Pass selectedPrice as a number
        } else {
            updateCart(productMain.id, productMain.quantityPurchase, colorToUse, selectedPrice); // Pass selectedPrice as a number
        }
                ...
                <div className="featured-product underwear md:py-20 py-10">
                    <div className="container flex justify-between gap-y-6 flex-wrap">
                        ...
                        <div className="product-infor md:w-1/2 w-full lg:pl-[15px] md:pl-2">
                            ...
                            <div className="text-title">Watt: <span className='text-title color'>{productMain.watt}W</span></div> 
                            <div className="text-title">Weight: <span className='text-title color'>{productMain.weight}Kg</span></div>    
                            <div className="choose-color">
    <div className="text-title">Size: <span className='text-title color'>{activeColor}</span></div>
    <div className="flex items-center mt-3">
    <ColorDropdown
    product={productMain} // Ensure this is the correct product data
    activeColor={activeColor} 
    onColorSelect={handleColorSelect} // Use your `handleActiveColor` method
/>
</div>
</div>
                                ...
                                <div className="more-infor mt-6">
                                    ...
                                    <div className="flex items-center gap-1 mt-3">
                                        ...
                                        <div className="text-secondary">{productMain.tag}</div>
                                    </div>
                                </div>
                            ...
                            <div className="get-it mt-6 pb-8 border-b border-line">
                                ...
                                <div className="item flex items-center gap-3 mt-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="caption1 text-secondary mt-1">Free shipping on orders over ₦100,000.</div>
                                    </div>
                                </div>
                                <div className="item flex items-center gap-3 mt-4">
                            ...
                            <div className={`desc-item description ${activeTab === 'description' ? 'open' : ''}`}>
                                ...
                                <div className="grid lg:grid-cols-4 grid-cols-2 gap-[30px] md:mt-10 mt-6">
                                    ...
                                    <div className="item">
                                        ...
                                        <div className="heading6 mt-4">Eco-Freindly Material</div>
                                        <div className="text-secondary mt-2">Use on walls, furniture, doors and many more surfaces. The possibilities are endless.</div>
                                    </div>
                                    <div className="item">
